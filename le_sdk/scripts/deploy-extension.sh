...
if m:
    try:
        # Unescape XML entities (e.g. &quot; -> \") before parsing JSON
        raw = m.group(1)
...
" "$manifest" "$ext_id" "$prefs_tmp" "$prefs_out"
  ...
  print_success "Extension registered — tap Refresh in Settings > Plugins to load"
}
...
if [ "$DEVICE_COUNT" -gt 1 ]; then
  ...
  print_warning "Multiple devices detected. Using: $FIRST_DEVICE"
  print_warning "Set ANDROID_SERIAL to target a specific device."
fi

DEVICE_NAME=$("$ADB" devices -l | grep 'device ' | head -1 | sed 's/.*model://' | sed 's/ .*//')
print_success "Device connected: ${DEVICE_NAME:-unknown}"
...
if [ "$DEPLOY_ON_DEVICE" = false ]; then
  ...
  if [ "$ALREADY_REGISTERED" = true ]; then
    print_success "Extension already registered at $SERVE_URL"
  else
  ...
  if [ "$NO_BUILD" = true ]; then
    ...
    (cd "$EXT_DIR" && flutter build web --no-web-resources-cdn --no-wasm-dry-run)
  ...
  if "$ADB" shell "ls '$DEVICE_PATH/pubspec.yaml' '$DEVICE_PATH/web/index.html'" > /dev/null 2>&1; then
    ...
    print_error "Verification failed — files may not have been pushed correctly."
    exit 1
  fi
  ...
  EXT_URL="file://$DEVICE_PATH/$ENTRY"

  ALREADY_REGISTERED=false
  PREFS_CHECK="$(mktemp)"
  if "$ADB" shell "run-as com.lattice.webview cat shared_prefs/FlutterSharedPreferences.xml" > "$PREFS_CHECK" 2>/dev/null; then
    if grep -q "$EXT_URL" "$PREFS_CHECK" 2>/dev/null; then
      ALREADY_REGISTERED=true
    fi
  fi
  rm -f "$PREFS_CHECK"
  ...
  if [ "$ALREADY_REGISTERED" = true ]; then
    print_success "Extension already registered (skipping registration)"
  else
    print_header "Registering Extension"
    register_extension "$EXT_URL"
  fi
  ...
  echo "  Extension registered — open it from the extension grid."
  echo ""
