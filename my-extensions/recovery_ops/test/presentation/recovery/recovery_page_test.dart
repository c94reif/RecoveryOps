...
void main() {
  ...
  testWidgets('RecoveryPage renders all fields and submit button',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());

    expect(
        find.widgetWithText(CustomTextField, 'Bumper Number'), findsOneWidget);
    ...
    expect(find.widgetWithText(CustomButton, 'Submit Recovery Request'),
        findsOneWidget);
  });
  ...
  testWidgets('Shows validation snackbar when both fields are empty',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());

    await tester
        .tap(find.widgetWithText(CustomButton, 'Submit Recovery Request'));
    await tester.pump();
  ...
  testWidgets('Shows validation snackbar when bumper number is empty',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());

    await tester.enterText(
        find.widgetWithText(CustomTextField, 'Issue'), 'Flat tire');
    await tester.pump();
  ...
  testWidgets('Shows validation snackbar when issue is empty',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());

    await tester.enterText(
        find.widgetWithText(CustomTextField, 'Bumper Number'), 'HQ-42');
    await tester.pump();
  ...
  testWidgets(
      'Submitting calls recoveryViewModel.submitRecoveryRequest with default Tow Bar',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
  ...
  testWidgets('Submitting with Wrecker type passes correct payload',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());

    await tester.enterText(
        find.widgetWithText(CustomTextField, 'Bumper Number'), 'BR-07');
    await tester.enterText(
        find.widgetWithText(CustomTextField, 'Issue'), 'Engine failure');
    await tester.pump();
  ...
  testWidgets('Fields clear after successful submit',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
  ...
  testWidgets('Whitespace-only input is treated as empty',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
  ...
  testWidgets('Mic buttons are rendered in each text field',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
  ...
  testWidgets('Idle mic buttons are green outlined with no background',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
  ...
  testWidgets(
      'Tapping mic switches it to red filled with red glow and updated tooltip',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
    ...
    expect(
        activeBtn.style!.shape!.resolve(<WidgetState>{}), isA<CircleBorder>());

    expect(find.byTooltip('Recording — tap to stop'), findsOneWidget);
    expect(find.byTooltip('Tap to record'), findsOneWidget);
  });

  testWidgets('Tapping the active mic again returns it to idle green',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
  ...
  testWidgets('Only the tapped field shows the recording state',
      (WidgetTester tester) async {
    await tester.pumpWidget(createWidgetUnderTest());
