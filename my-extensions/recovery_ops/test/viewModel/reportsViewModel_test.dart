...
class FakeMapService implements sdk.MapService {
  ...
  @override
  Future<String> addMarker(sdk.LatLng location, {String? label, String? color, sdk.MarkerIcon? icon, sdk.MarkerDisposition? disposition}) async {
  ...
  @override
  Future<void> addPolyline(String id, List<sdk.LatLng> points, {String? color}) async {
...
void main() {
  ...
  group('incoming messages', () {
    ...
    test('inserts at beginning of list', () async {
      vm.reports.insert(0, makeReport(bumperNumber: 'FIRST', isOutgoing: true, isRead: true));
      messaging.emitMessage(makeIncomingRecoveryMessage(bumperNumber: 'SECOND'));
      await Future.delayed(Duration.zero);
  ...
  group('navigator update messages', () {
    ...
    test('clears navigator data on navigation_stopped message', () async {
      final report = makeReport(
        id: 1,
        isOutgoing: true,
      ).copyWith(entityId: 'entity-123', navigatorLatitude: 34.5, navigatorLongitude: -85.5);
      vm.reports.add(report);
    ...
    test('clears navigator location in database on navigation_stopped', () async {
      final report = makeReport(
        id: 7,
        isOutgoing: true,
      ).copyWith(entityId: 'entity-123', navigatorLatitude: 34.5, navigatorLongitude: -85.5);
  ...
  group('navigateTo', () {
    ...
    test('updates report in list with result from NavigationViewModel', () async {
      final report = makeReport();
      navVm.navigateToResult = report.copyWith(
        navigatorLatitude: 34.0,
        navigatorLongitude: -85.0,
  ...
  group('deleteReport', () {
    test('publishes deletion for outgoing reports with entity id', () async {
      ...
      final report = makeReport(id: 1, isOutgoing: true)
          .copyWith(entityId: 'entity-1');
      vm.reports.add(report);
  ...
  group('yourReports and externalReports', () {
    ...
    test('all reports appear in exactly one filtered list', () {
      ...
      expect(vm.yourReports.length + vm.externalReports.length,
          vm.reports.length);
    });
  });
  ...
  group('unreadCount', () {
    ...
    test('counts only unread non-outgoing reports', () async {
      ...
      vm.reports.insert(0, makeReport(bumperNumber: 'C', isOutgoing: true, isRead: true));
      vm.updateUnread();
      expect(vm.unreadCount.value, 2);
    });
  });
  ...
  group('groupedExternalReports', () {
    ...
    test('groups reports into correct brackets', () {
      ...
      final grouped = vm.groupedExternalReports;
      expect(grouped[DistanceBracket.under1km]!.any((r) => r.bumperNumber == 'NEAR'), isTrue);
      expect(grouped[DistanceBracket.over20km]!.any((r) => r.bumperNumber == 'FAR'), isTrue);
    });
