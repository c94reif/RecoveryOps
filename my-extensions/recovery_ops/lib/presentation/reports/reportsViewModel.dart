...
export 'package:recovery_ops/domain/entities/recoveryReport.dart';
...
class ReportsViewModel extends ChangeNotifier {
  ...
  double? navigatorProgress(RecoveryReport report) {
    if (report.navigatorLatitude == null ||
        report.navigatorLongitude == null) return null;
  ...
  void applyNavigatorUpdate(NavigatorUpdate update) {
    final index = reports.indexWhere(
        (r) => r.entityId == update.entityId && r.isOutgoing);
    if (index == -1) return;
  ...
  void startGeometryPolling() {
    geometryPollTimer =
        Timer.periodic(pollInterval, (_) => pollForGeometry());
  }
