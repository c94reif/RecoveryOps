...
class ReportsDao extends DatabaseAccessor<AppDatabase> with _$ReportsDaoMixin {
  ...
  Future<List<ReportData>> getAllReports() {
    return (select(reports)..orderBy([(t) => OrderingTerm.desc(t.timestamp)]))
        .get();
  }
  ...
  Future<void> markAllAsRead() async {
    await (update(reports)..where((t) => t.isRead.equals(false))).write(
      const ReportsCompanion(isRead: Value(true)),
    );
  }
  ...
  Future<void> deleteReport(int id) async {
    await (delete(reports)..where((record) => record.id.equals(id))).go();
  }
}
